...
class P4_ModelBuilder() :
   ...
   def __init__(self, path_to_data=None, year=None) :
      ...
      self._dumpFile = 'oLinearDelayPredictor_save.dump'
   ...
   def print(self) :
      ...
      print("Fraction of data  ....................... :  "+str(self._fract_data))
   ...
   def _user_test_split(self,list_for_user_test=None) :
      ...
      # Features ORIGIN_CITY_NAME and DEST_CITY_NAME 
      # will be issued from df_route dataframe.
      # ------------------------------------------------------------------------ 
   ...
   #
   #----------------------------------------------------------------------------
   def compress(self, object) :
      ''' Compress object given as parameter using zlib object.'''
      # Compress:
      compressed = zlib.compress(pickle.dumps(object))
      return compressed
   #----------------------------------------------------------------------------
   ...
   def _data_for_computation(self,list_value_excluded = None) :
      ...
      For each one of the route :
         ...
         Features beloging to quantitative list are scaled then joined to encoded 
         features.
      ...
      for route in self.list_route :
         ...
         # ---------------------------------------------------------------------
         #self.oLinearDelayPredictor.encoder = preprocessing.OneHotEncoder()
         ...
         if X_std is not None :
            ...
            dict_route_model = {'encoder':encoder,'std_scale':std_scale}
            self.oLinearDelayPredictor._dict_route_model[route] \
            = dict_route_model.copy()
   ...
   def _model_compute(self, route, regresion_model_name, best_alpha) :
      ''' Evaluate model given as parameter.
      Evaluation means : 
      1) training model along with train data-set and testing model 
      along with test data-set. 
      
      2) Metrics such as R2, MAE, MSE are computed and stored into model-dictionary.
      3) Best hyper-parameter is stored into model dictionary.
      4) Model structure containing all model doefficients is stored into model-dictionary.
      ...
      #-------------------------------------------------------------
      dict_route_model = self._oLinearDelayPredictor._dict_route_model.copy()
      dict_model = dict_route_model[route]

      dict_model[regresion_model_name] = regresion_model
      self._oLinearDelayPredictor._dict_route_model[route] = dict_model.copy()
   ...
   def _model_build(self, dict_models_parameters) :
      ...
      Parameter linear_model is one of sklearn.linear_model library.
      '''
